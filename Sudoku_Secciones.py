!python -m pip install -i https://pypi.gurobi.com gurobipy
# Sudoku Secciones Basico
# Instrucciones de instalación: 
from gurobipy import * 
import numpy as np

# Creacion del modelo: Sudoku seccion basico
Matriz = np.zeros((9,9))
m = Model('Sudoku Secciones Basico')
I = np.arange(1,10)       #I = {1,..,9}
J = np.arange(1,10)       #J = {1,..,9}
K = np.arange(1,10)       #K = {1,..,9}

#Creacion de las variables
y = m.addVars(I,J,K, vtype = GRB.BINARY, name = "y")

# Restricciones
entradas = open("aij_seccionesb2_posiciones.csv", "r")
for lineas in entradas:
  fil = int(lineas[0])
  col = int(lineas[2])
  var = int(lineas[4])
  m.addConstr(y[fil,col,var] == 1 )

# Restricciones de fila y columna no repetidas
m.addConstrs(quicksum(y[i,j,k] for j in J) == 1 for i in I for k in K)
m.addConstrs(quicksum(y[i,j,k] for i in I) == 1 for j in J for k in K)

# Restriccioes de secciones irregulares
secciones = np.genfromtxt("aij_seccionesb_2.csv", delimiter = ",")
for r in range(1,10):
  tmp = [[],[]]
  ub = 9*r 
  lb = 9*(r-1)
  tmp[0] = secciones[:,0][lb : ub]
  tmp[1] = secciones[:,1][lb : ub]
  m.addConstrs(quicksum( y[tmp[0][l],tmp[1][l],k] for l in range(len(tmp[0])) ) == 1 for k in K )

# Restriccion cada celda debe de tener un valor entre 1 y 9
m.addConstrs(quicksum(y[i,j,k] for k in K ) == 1 for j in J for i in I)

m.setObjective(0 , GRB.MAXIMIZE)
m.optimize()
for i in I:
  for j in J:
    for k in K: 
      if y[i,j,k].X != 0:
        Matriz[i-1,j-1] = k   

print(Matriz)
